...
def load_data():
    with open('professors_data.json', 'r') as file:
        ...
        if not content:
            ...
            return {}
        print("File Content:", content)  # Log the content to check its validity
        try:
...
##### Comparison between temporal analysis of fields growth
    ...
    for field, publications in field_publications.items():
        years = sorted([year for year in publications.keys() if year is not None])
        base_year = min(years)
...
#### Publication Trends by Field
    ...
    def update_plot(selected_field):
        ...
        if selected_field:
            years = sorted([year for year in field_publications[selected_field].keys() if year is not None])
